...
public class Sessoes extends Dao{
	...
	public int qtosSessoesDisponiveis(int codEspetaculo) throws SQLException {
        	...
        	String query = "SELECT COUNT(e.codSessao)AS qtosSessoes FROM Sessoes s "
                        //+ "inner join Teatro t on t.codTeatro = s.codTeatro inner join Setor set on set.codTeatro = t.codTeatro inner join Assento a on a.codSetor = set.codSetor inner join Ingresso i on i.codAssento "
                        + "WHERE "
                        //+ "(SELECT COUNT(codAssento) FROM Assento) >  (SELECT COUNT(codIngresso) FROM Ingresso) and  "
                        + "s.codEspetaculo = ?";
	...
	public Sessao[] getSessoesDisponiveis(int codSessao) throws SQLException {
        	String query = "SELECT * FROM Sessao e "
                        //+ "inner join Sessao s on s.codSessao = e.codSessao inner join Teatro t on t.codTeatro = s.codTeatro inner join Setor set on set.codTeatro = t.codTeatro inner join Assento a on a.codSetor = set.codSetor inner join Ingresso i on i.codAssento "
                        + "WHERE "
                        //+ "(SELECT COUNT(codAssento) FROM Assento) >  (SELECT COUNT(codIngresso) FROM Ingresso) and "
                        + "s.codEspetaculo = ? ";
